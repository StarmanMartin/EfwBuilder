...
def walk(node) :
    for item in node.get_children():
        if item.is_collection:
            for res in walk(item):
                yield res
        else:
            yield item

def zip_folder(node):
    zip_subdir = node.get_filename()
    ...
    for fpath in walk(node.clone_as_root()):
        # Calculate path for file in zip
        zip_path = os.path.join(zip_subdir, fpath.get_path().strip('/'))

        # Add file, at correct path
        zf.write(fpath.get_abs_path(), zip_path)

    # Must close zip for all contents to be written
    zf.close()


    return (s.getvalue(), zip_filename)