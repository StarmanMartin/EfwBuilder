...
from WebDAV.utils import rfc3339_date, rfc1123_date, safe_join, urlquote
...
class BaseDavResource(object):
    ...
    root_extension = None
    root = '/'
    def __init__(self, path, root_extension: str = None):
        if root_extension is not None:
            root_extension = root_extension.strip('/')
            self.root_extension = root_extension
            self.root = os.path.join(self.root, root_extension)
        self.path = []
        path = path.strip("/")
        if path:
    ...
    def get_parent(self):
        return self.clone(path=self.get_parent_path())
    ...
    def copy_collection(self, destination, depth=-1):
        ...
        if depth != 0:
            for child in self.get_children():
                child.copy(self.clone(path=safe_join(destination.get_path(), child.displayname)),
                           depth=depth-1)
    ...
    def move_collection(self, destination):
        ...
        for child in self.get_children():
            child.move(self.clone(path=safe_join(destination.get_path(), child.displayname)))
        self.delete()

    def clone(self, **kwargs):
        root_extension = kwargs.get('root_extension', self.root_extension)
        path = kwargs.get('path', self.get_path)
        return self.__class__(path=path, root_extension=root_extension)

    def clone_as_root(self):
        root_extension = self.root_extension or '/'
        path = self.get_path()
        return self.__class__(path="/", root_extension=os.path.join(root_extension, path))
